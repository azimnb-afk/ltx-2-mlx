...
__all__ = ["BlockLoraSource", "BlockStreamer", "GGUFBlockStreamer", "StreamingLTXModel"]
...
def _remap_gguf_block_param(name: str) -> str:
    name = name.replace(".to_out.0.", ".to_out.")
    name = name.replace(".net.0.proj.", ".proj_in.")
    name = name.replace(".net.2.", ".proj_out.")
    name = name.replace(".linear_1.", ".linear1.")
    name = name.replace(".linear_2.", ".linear2.")
    return name


class GGUFBlockStreamer:
    """Stream transformer block weights directly from a GGUF checkpoint.

    Dequantizes blocks on demand (e.g. Q4_K_M / Q6_K) into a single shared
    block module, so peak memory usage stays minimal (~1 block) while maintaining
    ...
    def __init__(self, gguf_path: str | Path) -> None:
        ...
        self.reader = gguf.GGUFReader(self.gguf_path)
    ...
    def bind(
        ...
        *,
        ...
        block_weights = {}
        for name, t in self.tensor_map.items():
            if name.startswith(prefix):
                param_name = _remap_gguf_block_param(name[len(prefix):])
                if t.tensor_type == gguf.GGMLQuantizationType.F32:
                    arr = t.data
                else:
                    arr = dequantize(t.data, t.tensor_type)
                block_weights[param_name] = mx.array(arr.astype(np.float32)).astype(mx.bfloat16)

        block.load_weights(list(block_weights.items()), strict=False)
