...
class LatentState:
    ...
    Attributes:
        ...
        keyframes_mask: Generated-keyframe marker (B, N, 1), or None.
    """
...
def extend_keyframes_mask(mask: mx.array | None, num_tokens: int) -> mx.array | None:
    """Append non-keyframe entries when conditioning tokens extend a sequence."""
    if mask is None:
        return None
    zeros = mx.zeros((mask.shape[0], num_tokens, 1), dtype=mask.dtype)
    return mx.concatenate([mask, zeros], axis=1)
...
class VideoConditionByLatentIndex:
    ...
    def apply(self, state: LatentState, spatial_dims: tuple[int, int, int]) -> LatentState:
        ...
        if new_keyframes_mask is None:
            new_keyframes_mask = mx.zeros(
        ...
        for i, frame_idx in enumerate(self.frame_indices):
            ...
            keyframe_marker = mx.ones((state.latent.shape[0], tokens_per_frame, 1), dtype=new_keyframes_mask.dtype)
            new_keyframes_mask = mx.concatenate(
                [new_keyframes_mask[:, :start, :], keyframe_marker, new_keyframes_mask[:, end:, :]], axis=1
            )
