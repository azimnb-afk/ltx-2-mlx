...
from ltx_core_mlx.conditioning.types.latent_cond import LatentState, extend_keyframes_mask
...
class VideoConditionByKeyframeIndex:
    ...
    def apply(self, state: LatentState, spatial_dims: tuple[int, int, int]) -> LatentState:
        ...
        return LatentState(
            ...
            # These are given-content conditioning tokens, not generated
            # keyframe slots. Official LTX marks appended references false.
            keyframes_mask=extend_keyframes_mask(state.keyframes_mask, num_kf),
        )
