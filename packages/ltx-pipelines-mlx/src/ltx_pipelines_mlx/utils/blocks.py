...
class ImageConditioner:
    ...
    def load(self) -> _VideoVAEEncoder:
        ...
        Mirrors :meth:`VideoDecoder.load` because the two halves ship in the
        same artifacts and must survive the same conventions. In particular the
        official LTX-2.5 combined VAE file
        (``ltx-2.5-video-vae-conv-bf16.safetensors``) stores *both* halves under
        ``encoder.``/``decoder.`` prefixes, so the LTX-2.3 ``vae_encoder.``
        prefix matches nothing there. That mismatch used to be invisible: this
        method loaded with ``strict=False``, so zero matched keys left a fully
        random-initialized encoder, and every I2V generation silently encoded
        its reference image through noise. The output was a valid MP4 of brown
        mush rather than a visible failure. ``strict=True`` below is what makes
        that class of bug impossible to reintroduce.
        """
        if self._encoder is not None:
            return self._encoder
        encoder = _VideoVAEEncoder()
        vae_enc_path = _resolve_component_file(
            self.model_dir, "vae_encoder.safetensors", extra_stems=["video-vae-conv", "vae_encoder"]
        )
        raw_weights = load_split_safetensors(vae_enc_path)
        if any(k.startswith("vae_encoder.") for k in raw_weights):
            # LTX-2.3 split-safetensors convention: encoder-only file,
            # "vae_encoder." prefix on every key.
            weights = load_split_safetensors(vae_enc_path, prefix="vae_encoder.")
            weights = {
                k.replace("._mean_of_means", ".mean_of_means").replace(
                    "._std_of_means", ".std_of_means"
                ): v
                for k, v in weights.items()
            }
        elif any(k.startswith("encoder.") or k.startswith("decoder.") for k in raw_weights):
            # Official LTX-2.5 combined VAE file: encoder + decoder in one file.
            # Only the encoder half is used here; decoder.* keys are dropped
            # (this class never runs the decoder). The per-channel stats are
            # published as "*-of-means"; this encoder's modules expect
            # "mean_of_means"/"std_of_means" (the decoder half instead expects
            # the bare "mean"/"std", which is why the rename differs from
            # VideoDecoder.load's).
            stat_renames = {
            ...
            # Same deterministic PyTorch->MLX Conv3D reorder the decoder half
            # needs, applied under the same guard: only to a tensor whose stored
            # shape does not already match but does match after the known
            # (O,I,D,H,W) -> (O,D,H,W,I) permutation. Anything else fails closed
            # rather than being guessed at.
            expected_shapes = {k: tuple(v.shape) for k, v in tree_flatten(encoder.parameters())}
            weights = {}
            for target_key, v in remapped.items():
                ...
                raise ValueError(
                    ...
                    f"stored shape {stored_shape} matches this encoder's expected "
