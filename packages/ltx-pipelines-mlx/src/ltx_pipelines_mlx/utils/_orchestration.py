...
def transformer_pack_is_quantized(transformer_path: str | Path) -> bool:
    ...
    with safe_open(str(transformer_path), framework="numpy") as handle:
        return any(key.endswith(".scales") for key in handle)
...
def load_transformer(
    ...
    *,
    ...
    gguf_path = None
    if transformer_path.is_file() and transformer_path.suffix == ".gguf":
        gguf_path = transformer_path
    elif transformer_path.is_dir():
        ggufs = list(transformer_path.glob("*.gguf"))
        if ggufs:
            gguf_path = ggufs[0]
    elif not transformer_path.exists():
        ggufs = list(transformer_path.parent.glob("*.gguf"))
        if ggufs:
            gguf_path = ggufs[0]

    if gguf_path:
        from ltx_core_mlx.loader.block_streaming import GGUFBlockStreamer, StreamingLTXModel, _remap_gguf_block_param
        import gguf
        from gguf.quants import dequantize
        import numpy as np

        config = LTXModelConfig.from_checkpoint_dir(gguf_path)
        dit = LTXModel(config)
        dit.transformer_blocks = [dit.transformer_blocks[0]]

        streamer = GGUFBlockStreamer(gguf_path)
        non_block_weights = {}
        for name, t in streamer.tensor_map.items():
            if not name.startswith("transformer_blocks."):
                clean_name = name[len("transformer."):] if name.startswith("transformer.") else name
                clean_name = _remap_gguf_block_param(clean_name)
                if t.tensor_type == gguf.GGMLQuantizationType.F32:
                    arr = t.data
                else:
                    arr = dequantize(t.data, t.tensor_type)
                non_block_weights[clean_name] = mx.array(arr.astype(np.float32)).astype(mx.bfloat16)

        dit.load_weights(list(non_block_weights.items()), strict=False)
        dit = StreamingLTXModel(dit, streamer)
        mx.eval(dit.parameters())
        aggressive_cleanup()
        return dit

    config = LTXModelConfig.from_checkpoint_path(transformer_path)
    dit = LTXModel(config)
...
def decode_and_save_video(
    ...
    *,
    ...
    """
    audio_path = None
    if audio_decoder is not None and audio_latent is not None:
        try:
            if getattr(audio_latent, "ndim", 0) == 4 or (isinstance(audio_latent, mx.array) and len(audio_latent.shape) == 4):
