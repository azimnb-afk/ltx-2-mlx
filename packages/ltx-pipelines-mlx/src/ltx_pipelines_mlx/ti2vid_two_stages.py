...
class TI2VidTwoStagesPipeline(BasePipeline):
    ...
    def _load_upsampler(self) -> None:
        ...
        from ltx_pipelines_mlx.utils.blocks import _resolve_component_file

        weights_path = _resolve_component_file(
            self.model_dir,
            "spatial_upscaler_x2_v1_1.safetensors",
            extra_stems=["spatial_upscaler_x2", "spatial-upscaler-x2"],
        )
    ...
    def generate_and_save(
        ...
        *,
        ...
        ``disable_audio`` skips loading the audio decoder/vocoder and mux
        step entirely -- the audio latent is still produced by the joint
        transformer forward (there's no cheap way to skip that half of a
        single joint audio+video attention pass), but nothing downstream of
        it runs, and the output file has no audio stream.
        """
        ...
        self._load_decoders(disable_audio=disable_audio)

        result = self._decode_and_save_video(
            video_latent, audio_latent, output_path, frame_rate=frame_rate, disable_audio=disable_audio
        )
