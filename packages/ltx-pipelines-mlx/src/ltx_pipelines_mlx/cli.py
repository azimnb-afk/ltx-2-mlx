...
""",
    ...
    gen.add_argument(
        ...
        action="store_true",
        dest="no_audio",
        help="Skip audio decoding and muxing (video-only output). The joint transformer forward "
        "still produces an audio latent internally (it can't be split out of a single joint "
        "audio+video attention pass), but the audio VAE decoder and vocoder are never loaded and "
        "the output file has no audio stream. Mirrors --no-regen-audio's naming on `retake`.",
    )
...
def _cmd_generate(args: argparse.Namespace) -> None:
    ...
    if args.one_stage:
        ...
        if not args.quiet:
            ...
            is_ltx25 = False
            try:
                cfg_loaded = LTXModelConfig.from_checkpoint_dir(Path(args.model), strict=False)
                if getattr(cfg_loaded, "model_version", None) == (2, 5):
                    is_ltx25 = True
            except Exception:
                pass
            if is_ltx25:
                print("Mode: LTX-2.5 Distilled One-Stage (full target resolution 8-step)")
            else:
