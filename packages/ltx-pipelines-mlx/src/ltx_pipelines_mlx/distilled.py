...
class DistilledPipeline(TI2VidTwoStagesPipeline):
    ...
    def load(self) -> None:
        ...
        if getattr(self.dit.config, "model_version", None) != (2, 5):
    ...
    def generate_two_stage(  # type: ignore[override]
        ...
        **_unused_kwargs,
        ...
        # Check for LTX-2.5 1-stage distilled model
        if getattr(self.dit.config, "model_version", None) == (2, 5):
            ...
            output = denoise_loop(
                ...
                diffusion_step=None, # Verified pure Euler distilled flow
            )
            video_latent_5d = self.video_patchifier.unpatchify(
                output.video_latent[:, : F * H * W, :], (F, H, W)
            )
            audio_latent_4d = self.audio_patchifier.unpatchify(output.audio_latent)
            return video_latent_5d, audio_latent_4d
